<?php
/*Template Name: ー第1次川中島の戦い */
?>
<?php get_header(); ?>

<div class="c-herohead">
	<h1 class="c-herohead__heading"><span><?php the_title(); ?></span>八幡の戦い・布施の戦い<span>信玄と謙信の初めての戦いは、 互いの力のさぐり合い</span></h1>
</div>

<section class="c-section">
	<div class="p-illusts__post__kv">
		<img src="<?php echo get_template_directory_uri(); ?>/assets/img/illusts/01_01.jpg" alt="川中島合戦">
	</div>
	<article class="c-article c-single p-illusts__post">
		<div class="c-single__block">
			<h2 class="wp-block-heading">第1次川中島の戦い （壱）</h2>
			<h3 class="wp-block-heading">武田軍と上杉軍、最初の衝突！更級郡八幡で、上杉方の越後·北信濃連合軍が勝利</h3>
			<ul>
				<li>天文22年(1553)4月9日、武田信玄(当時は武田晴信と名のる)の北信濃侵攻により、村上義清は葛尾城を追われ、北信濃の豪族、高梨·井上·島津·須田·栗田氏らとともに越後の上杉謙信(当時は長尾景虎と名のる)を頼る。</li>
				<li>求めに応じた上杉謙信は信濃に出陣。①4月22日、村上義清らとの連合軍を率いて、更級郡八幡で武田軍と初めて交戦する。【八幡の戦い】武田軍は敗れ、上杉勢は葛尾城を奪回、村上義清は奪われた旧領を回復する。</li>
			</ul>
	</article>
	<div class="p-illusts__post__kv">
		<img src="<?php echo get_template_directory_uri(); ?>/assets/img/illusts/01_02.jpg" alt="川中島合戦">
	</div>
	<article class="c-article c-single p-illusts__post">
		<div class="c-single__block">
			<h2 class="wp-block-heading">第1次川中島の戦い （弐）</h2>
			<h3 class="wp-block-heading">武田軍、東信濃の諸城を次々と攻略し、怒濤の進撃を開始<br>村上義清は旧領回復もつかの間、越後へと敗走する</h3>
			<ul>
				<li>武田信玄は一旦甲斐に兵を引き上げたが、7月に、再軍備をもって再び佐久に侵攻し、次々と城を攻略し、東信濃を制圧していく。</li>
				<li>8月、小県郡塩田城(上田市)に立てこもっていた村上義清は越後に敗走し、再び上杉謙信に援軍を求める。武田方の手に落ちた塩田城は、その後、飯富虎昌[おぶとらまさ]を城代として川中島への前進拠点となる。</li>
			</ul>

			<h3 class="wp-block-heading">謙信、布施と八幡の戦いで武田軍を破り、東信濃方面に進出<br>山城の争奪戦を繰り広げた後、両軍とも兵をひく</h3>
			<ul>
				<li>②8月の末、春日山城から信濃に出陣した上杉謙信は、更級郡布施(長野市篠ノ井)で武田軍と交戦する。③9月1日には更級郡八幡「やわた/更埴市八幡]に進軍し、武田方の荒砥城(千曲市)を自落させる。</li>
				<li>9月3日、上杉軍は筑摩郡の青柳城[あおやぎじょう](筑北村[ちくほくむら])に放火し、会田虚空蔵山城[あいだこくうぞうざんじょう](松本市四賀)を攻略。塩田城(上田市)で指揮をとっていた武田信玄は、上杉の手に落ちた麻績城[おみじょう](麻績村)·荒砥城を焼き討ちさせるなど、東信濃の筑北地域で小競り合いが続いた。</li>
				<li>9月17日、上杉軍は埴科郡にも進出し坂木南条に放火。武田勢を攻め、信玄は塩田城から兵を出して迎え撃とうとする。それを知った上杉謙信は川中島に後退して、そのまま越後に引き上げた。このとき上杉謙信は善光寺の仏像·仏具を春日山城下に運んだといわれる。</li>
				<li>武田信玄もまた善光寺平への北上を断念して、塩田城から深志城(現·松本城)に入り、甲州に帰陣した。</li>
			</ul>
		</div>
	</article>
</section>

<div class="c-nav__footnav">
	<div class="c-nav__footnav__next"><a href="<?php echo home_url('/illusts/2nd'); ?>"><span><i>第2次川中島の戦い</i></span></a></div>
</div>
<div class="c-single__back"><a class="c-anchor__back" href="<?php echo home_url('/illusts'); ?>"><span>川中島の戦い一覧へ戻る</span></a></div>

<?php get_footer(); ?>
